...
const SEASONS = ['2526','2425','2324','2223','2122','2021','1920','1819','1718','1617','1516'];
...
async function searchAll(query, field, seasonFilter){
    ...
    for(const s of SEASONS){
        ...
        for(let i=1;i<=38;i++) candidates.push(`${s}_liga_${pad(i)}.json`);
        // champions
        candidates.push(`${s}_cham.json`);
        for(let i=1;i<=14;i++) candidates.push(`${s}_cham_${pad(i)}.json`);
        // copa
        candidates.push(`${s}_copa.json`);
        for(let i=1;i<=10;i++) candidates.push(`${s}_copa_${i}.json`);        
        // supercopas
        candidates.push(`${s}_sces.json`);
        for(let i=1;i<=5;i++) candidates.push(`${s}_sces_${i}.json`);
        candidates.push(`${s}_sceu.json`);
        for(let i=1;i<=5;i++) candidates.push(`${s}_sceu_${i}.json`);
        // mundialito
        candidates.push(`${s}_mund.json`);
        for(let i=1;i<=10;i++) candidates.push(`${s}_mund_${i}.json`);
        // intercontinental
        candidates.push(`${s}_intc.json`);
        for(let i=1;i<=10;i++) candidates.push(`${s}_intc_${i}.json`);
        candidates.push(`${s}_index.json`);
